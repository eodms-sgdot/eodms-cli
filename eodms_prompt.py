...
class _PromptConsoleFilter(logging.Filter):
    def filter(self, record):
        # Keep package logs visible on stdout, but avoid echoing entries
        # generated by the prompt's print() override.
        return not getattr(record, 'from_prompt_print', False)
...
def print(*args, **kwargs):
    ...
    logger.log(log_level, plain_text, extra={'from_prompt_print': True})
...
class Prompter:
    ...
    def ask_filter(self, filters):
        ...
        def _build_example_predicate(field_name, field_meta):
            if not isinstance(field_meta, dict):
                return f"{field_name} = 'example'"

            field_type = _first_type(field_meta.get('type'))
            enum_values = field_meta.get('enum')
            field_format = str(field_meta.get('format') or '').lower()

            if isinstance(enum_values, list) and len(enum_values) > 0:
                enum_val = enum_values[0]
                if isinstance(enum_val, str):
                    return f"{field_name} = '{enum_val}'"
                return f"{field_name} = {enum_val}"

            if field_type in ('integer', 'number'):
                min_val = field_meta.get('minimum')
                if min_val is None:
                    min_val = 1
            ...
            if field_format in ('date-time', 'date') or field_name in ('datetime', 'start_datetime', 'stop_datetime'):
                return f"{field_name} >= DATE('2024-01-01')"

            if field_name.lower().find('orbit') > -1:
                return f"{field_name} = 1"

            return f"{field_name} LIKE 'example%'"

        def _print_local_queryables(queryables):
            if not isinstance(queryables, dict) or len(queryables) == 0:
                print("  No queryable fields advertised by this collection.")
                return

            for field_name in sorted(queryables.keys()):
                field_meta = queryables.get(field_name)
                field_type = _first_type(field_meta.get('type') if isinstance(field_meta, dict) else None)
                example = _build_example_predicate(field_name, field_meta)

                constraints = []
                if isinstance(field_meta, dict):
                    enum_values = field_meta.get('enum')
                    if isinstance(enum_values, list) and len(enum_values) > 0:
                        preview = ', '.join(str(v) for v in enum_values[:5])
                        suffix = '...' if len(enum_values) > 5 else ''
                        constraints.append(f"enum=[{preview}{suffix}]")
                    if field_meta.get('pattern'):
                        constraints.append(f"pattern={field_meta.get('pattern')}")
                    if field_meta.get('minimum') is not None or field_meta.get('maximum') is not None:
                        constraints.append(
                            f"min={field_meta.get('minimum')} max={field_meta.get('maximum')}"
                        )

                line = f"* {field_name} ({field_type}) e.g. {example}"
                if len(constraints) > 0:
                    line += f" | constraints: {'; '.join(constraints)}"

                print(self.wrap_text(line, init_indent='  ', sub_indent='    '))

        def _build_collection_example(queryables):
            if not isinstance(queryables, dict) or len(queryables) == 0:
                return ""
            ...
            selected = []
            lower_map = {k.lower(): k for k in queryables.keys()}
            for preferred in candidate_names:
                original = lower_map.get(preferred)
                if original:
            ...
            if len(selected) == 0:
                selected = sorted(queryables.keys())[:2]

            predicates = []
            for field_name in selected:
                predicates.append(_build_example_predicate(field_name, queryables.get(field_name)))
        ...
        if filters is None:
            ...
            if not self.eod.silent:
                ...
                for coll in self.params['collections']:
                    ...
                    self.eod.print_queryables(coll_id)
                    queryables = av_fields.get('results', {})

                    example_filter = _build_collection_example(queryables)
                    if example_filter:
                        print(f"Suggested CQL2 for '{coll}':")
                        print(self.wrap_text(example_filter, init_indent='  '))
...
def setup_logger(log_name, log_path, level=PROMPT_DEFAULT_LOG_LEVEL,
    ...
    for handler in logger.handlers:
        if isinstance(handler, handlers.RotatingFileHandler):
            ...
            if existing_path and os.path.abspath(existing_path) == target_path:
                ...
                not isinstance(handler, handlers.RotatingFileHandler):
    ...
    if existing_file_handler is None:
        ...
        existing_file_handler = log_handler
    ...
    has_prompt_console_filter = any(
        isinstance(filt, _PromptConsoleFilter)
        for filt in getattr(existing_stream_handler, 'filters', [])
    )
    if not has_prompt_console_filter:
        existing_stream_handler.addFilter(_PromptConsoleFilter())
